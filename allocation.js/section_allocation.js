...
// ---- Subject Allocation (revised) ----
// - Balances subjects across ALL 5 weekdays (not a restricted 3-day window).
// - Enforces "max 1 slot of a given subject per day" in BOTH the balanced pass
//   and the fallback pass (previously only the balanced pass checked this).
// - Faculty/room conflicts are checked via the shared trackers so labs and
//   subjects across every section stay mutually consistent.
function placeSubjects(sections, classRooms, facultyBookings, classRoomBookings) {
    for (const section of sections) {
        ...
        for (const subject of subjectsPool) {
            ...
            // but still respect the max-1-per-day rule.
            if (!placed) {
